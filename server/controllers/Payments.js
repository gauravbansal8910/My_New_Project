...
exports.capturePayment = async (req, res) => {
    ...
    const userId = req.user.id;     // mtlb user hum koi user input se nahi extract kr rhe hum directly ussi user ko le rhe hai jo login krke baitha hai toh hum token se extract kr rhe hai
    //validation
    //valid courseID
    try{
    if(courses.length === 0) {
    ...
    for(const course_id of courses){
        ...
        try{
            ...
            const uid = new mongoose.Types.ObjectId(userId);        // new mongoose.Types.ObjectId(userId) is used to create a MongoDB ObjectId from a string userId
        ...
        const options = {
            ...
            currency: "INR",
            receipt: `${Math.random()}_${Date.now()}`,      // to make it unique
...
exports.verifySignature = async (req, res) => {
        ...
        try{
            //verify the signature
            // The user's secret key would not be valid or available for this purpose since it's the admin's account that has the credentials to communicate securely with Razorpay's API and verify transactions.
